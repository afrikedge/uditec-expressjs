    ...
    if(recoveryActivityNo){
        ...
        .then((result1) => {
            if(result1.recordset.length>0){
                ...
                request.query("select * from [PROD].[RecoveryActivityLine] where [Activity No_]= @inputField")
                .then(result2 => {
...
exports.getAssignedCustomerList = ('/getAssignedCustomerList',(req, res, next) => {
    const request = new sql.Request();
    request.query(`select * from [PROD].[AssignedCustomer]`)
    .then((result) => {
        if(result.recordset.length>0){
            res.status(200).json(result.recordset)
        }else{
...
exports.getAssignedCustomerCard = ('/getAssignedCustomerCard',(req, res, next) => {
    const assignedCustomerNo = req.query.assignedCustomerNo
    if(assignedCustomerNo){
        const request = new sql.Request();
        request.input('inputField', sql.VarChar, assignedCustomerNo);
        request.query("select * from [PROD].[AssignedCustomer] where [No_]=@inputField " )
        .then((result) => {
            if(result.recordset.length>0){
                res.status(200).json(result.recordset[0])
            }else{
        ...
        res.status(400).json({message:"Paramètre <assignedCustomerNo> absent de la requête Http"})
    }
});

exports.getActivityFeedbackList = ('/getActivityFeedbackList',(req, res, next) => {
    const request = new sql.Request();
    request.query("select * from [PROD].[ActivityFeedback]" )
    .then((result) => {
        res.status(200).json(result.recordset)
    })
    .catch((err) => {res.status(500).json({ err });console.log(err)}); 
